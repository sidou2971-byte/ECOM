            ...
            <div class="royal-card mb-8">
                <div class="space-y-6">
                    @foreach($cartItems as $item)
                        <div class="flex flex-col md:flex-row gap-6 pb-6 last:border-0" style="border-bottom: 2px solid rgba(201,168,76,0.15);">
                            <!-- Product Image -->
                            <div class="w-full md:w-40 h-40 rounded-lg overflow-hidden bg-gray-100 flex items-center justify-center flex-shrink-0">
                            ...
                            <!-- Product Info -->
                            <div class="flex-1">
                                <h3 class="text-lg font-bold mb-1 font-cinzel" style="color: var(--gold);">
                                    {{ $item->product->name }}
                                </h3>
                                <p class="text-gray-500 mb-3 text-sm">{{ \Illuminate\Support\Str::limit($item->product->description, 80) }}</p>
                                ...
                                <div class="flex items-center justify-between flex-wrap gap-4">
                                    ...
                                    <div class="flex items-center gap-4">
                                        ...
                                        <form action="{{ route('cart.update', $item->id) }}" method="POST" class="flex items-center gap-2">
                                            @csrf
                                            @method('PUT')
                                            <label class="font-semibold text-sm">Qté:</label>
                                            <input type="number"
                                                   name="quantity"
                                                   value="{{ $item->quantity }}"
                                                   min="1"
            ...
            <div class="royal-card">
                <div class="flex flex-col md:flex-row justify-between items-center gap-6">
                    ...
                    <div class="flex gap-4">
                        ...
                        <a href="{{ route('login') }}" class="royal-btn">
